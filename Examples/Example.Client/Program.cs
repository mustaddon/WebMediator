...
// file upload request
await mediator.Send(new FileUpload
{
...
using var downloadResponse = await mediator.Send(new FileDownload { Name = "example.txt" });
...
var downloadResponse2 = await mediator.Send(new FileDownloadWithInfo { Name = "example.txt" });
